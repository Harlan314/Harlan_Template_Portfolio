...
My last Milestone was comprehensively reading the code and modifying it so that I could implement my modifications, which included a 7-segment display score counter and a buzzer that plays noises whenever you get a point and gives a long tone when you win/reach score 10. Setting up the buzzer to give a long tone for winning the game was actually much more difficult than I expected, since for some reason putting a delay on the buzzer before turning it off always managed to break the system that lets you restart the game by clicking on the joystick. I eventualy resolved this problem by making and calling a function that controlled the long buzzer tone. I also modified the project by making it run off of a battery rather than a computer, so it is easier to move around.
